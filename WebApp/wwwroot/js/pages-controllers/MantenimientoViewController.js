...
// Estados estan automaticos, y los calcula en tiempo real basado en fecha y hora.
// Paso 1: recalcula el estado de cada mantenimiento y lo actualiza si cambió.
// Paso 2: SIEMPRE (aunque el mantenimiento no haya cambiado) revisa turbina por
// turbina si tiene algun mantenimiento "En Proceso" ahora mismo. Esto hace falta
// porque un mantenimiento recien creado ya nace con su estado final correcto
// (ej. "En Proceso"), entonces nunca se detectaria como "cambio" en el paso 1,
// y la turbina nunca se actualizaria si solo dependiera de ese paso.
function SincronizarEstados(callback) {
        ...
        if (estadoCalculado !== m.estadoMantenimiento) {
            ...
            pendientes.push(
                $.ajax({
                    ...
                    console.error("Fallo al actualizar Mantenimiento " + m.id, jqXHR.responseText);
                })
            );
        }
    });
    ...
    $.each(window.turbinasList, function (i, turbina) {
        ...
        if (tieneMantenimientoEnProceso && turbina.estado !== "Mantenimiento") {
            ...
            pendientes.push(
                ActualizarEstadoTurbina(turbina).fail(function (jqXHR) {
                    console.error("Fallo al actualizar Turbina " + turbina.id, jqXHR.responseText);
                })
            );
        } else if (!tieneMantenimientoEnProceso && turbina.estado === "Mantenimiento") {
